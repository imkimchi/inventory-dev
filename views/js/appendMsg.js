...
async function appendElements (username, AllConvos) {
    ...
    for(let convo of AllConvos.data) {
        ...
        let AllMessages = await axios.get(`/message/list/${convo._id}`)
            ...
            if(buyOrsell(sender) === 'sell') {
                ...
                if(convo.offer.offered) {
                    ...
                    console.log("not offered", convo)
                }
            }

            if(buyOrsell(sender) === 'buy') {
                console.log("buy")
                if(convo.sold) {
                    console.log('sold')
        ...
        //결제 됬을때만
        console.log("buyorsell!", buyOrsell(sender))

        if(convo.sold) {
            $convo.find('.conversation-listing-image').css('opacity', 0.5)
            $convo.find('.conversation-item-cta').prepend($('<div class="notice-item"><h1 class="notice-text">SOLD OUT</h1></div>'))
            ...
            for(let message of AllMessages.data) {
                ...
                let $acceptmessage = $(`<div class="conversation conversation-${isSenderMe(sender)}"> <div class="conversation-wrap"> <div class="conversation-image"><img src="..${profilePic}"></div> <div class="sub-title"> ${sender} </div> </div> <div class="messages" data-id="${message._id}"> <div id="acceptOffer">Accept Offer</div><div class="date"> ${sent_date} </div> </div> </div>`)
